...
1.  对于闭包，可以使用弱引用
2.  对于循环引用，可以使用 WeakMap
3.  对于回调函数，可以使用 WeakSet
4.  对于定时器，可以使用 clearTimeout
5.  对于事件监听器，可以使用 removeEventListener

# cache control 有哪些方式

1.  max-age 从响应发出开始计算最多能用多少秒，如果没过期则直接使用缓存
2.  no-cache 缓存，但每次使用前必须验证
3.  no-store 不缓存，直接请求最新资源
4.  only-if-cached 如果本地有缓存，则使用缓存，否则请求最新资源

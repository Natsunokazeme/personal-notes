...
把全局代码拆成独立模块，每个模块有自己的作用域、明确的输入输出，让依赖关系在编译时就能确定。
解决了问题：所有变量都挂在 window 上 → 名称冲突；文件加载顺序有依赖；无法封装、复用；难以管理大型项目。
实现了代码拆分、作用域隔离、依赖管理、复用。

# class 组件和 function 组件区别

class 组件会在 fiber 节点上创建实例(this)，通过 this.state 和 this.setState() 管理状态

function 组件通过 fiber 节点保存并重新执行函数，没有实例，继承等，通过 hooks 管理状态

# zustand,mobx,jutai,redux 等状态管理库的区别

zustand 极简，轻量、高性能，不依赖于 react，适合中小型项目
redux 生态丰富，适合大型项目
jutai 原子化状态管理，灵活性高
